...
class ImageData:
    ...
    def plot_area_spectrum(self,
        ...
        logger.info(f"[ImageData] Plotting mean spectrum of a {pixels_per_dimension} by {pixels_per_dimension} area starting at {top_right_coordinate}")
    ...
    def mean_spectrum_area(
        ...
        logger.info(f"[ImageData] Calculating mean spectrum for area x: {corner_coords[0]}-{corner_coords[0] + pixels_per_dimension}, y: {corner_coords[1]}-{corner_coords[1] + pixels_per_dimension}...")
    ...
    def register_bands(self, other: ImageData | int, band_mask_paths_ref: list[str | None] | None = None, band_mask_paths_src: list[str | None] | None = None) -> ImageData|None:
        ...
        if type(other) is not int:
            ...
            if self.nbands != other.nbands:
                logger.info(f"[ImageData] Provided ImageData instances have incompatible number of bands ({self.nbands} vs {other.nbands})")
        ...
        registered_img_data = np.zeros((self.img_data.shape[0], self.img_data.shape[1], self.img_data.shape[2]), dtype=np.float64)

        for i_band in range(self.nbands):
            logger.info(f"[ImageData] Registering band {i_band} out of {self.nbands}...")

            registered_img_data[:, :, i_band] = self._register_band(other, i_band, band_mask_paths_ref, band_mask_paths_src)
    ...
    def _register_band(self, other: ImageData | int, i_band: int, band_mask_paths_ref: list[str | None] | None, band_mask_paths_src: list[str | None] | None) -> np.ndarray | None:
        ...
        p1 = np.float32([kp_ref[m.queryIdx].pt for m in good_matches]) #.reshape(-1, 1, 2)
        p2 = np.float32([kp_src[m.trainIdx].pt for m in good_matches]) #.reshape(-1, 1, 2)
        
        img_matches = cv.drawMatches(img_ref_calc, kp_ref, img_src_calc, kp_src, good_matches, None, flags=cv.DrawMatchesFlags_NOT_DRAW_SINGLE_POINTS)
